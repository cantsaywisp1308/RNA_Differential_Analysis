...
for i in {16..18}
do
	   NAME="3164_${i}_S${i-12}_"
	      INPUT1="raw_data/illumina_30/${NAME}R1_001.fastq.gz"
	         INPUT2="raw_data/illumina_30/${NAME}R2_001.fastq.gz"
		    # Check if input files exist
		       if [[ -f "$INPUT1" && -f "$INPUT2" ]]; then
			             mkdir -p output
				           a="bowtie2 -x index/var_para_EPS -1 ${INPUT1} -2 ${INPUT2} -S output/illumina_30/${NAME}.sam --no-unal"
					   echo $a
					   eval $a
					      else
